...
export default class CCFieldFormatter {
  ...
  formatValues = (values) => {
    ...
    return pick({
      ...
      birth: this._formatBirth(values.birth),
      password: this._formatPassword(values.password),
  ...
  _formatBirth = (birth) => {
    const maxCVCLength = 6;
    return limitLength(removeNonNumber(birth), maxCVCLength);
  };

  _formatPassword = (password) => {
    const maxCVCLength = 2;
    return limitLength(removeNonNumber(password), maxCVCLength);
  };
