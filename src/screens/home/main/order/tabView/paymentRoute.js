...
const PaymentRoute = (props) => {
  ...
  const calcRecurringPay = () => {
    let recurringPlusMonth = Number(recurringMonthSelect[0]);
    let year = (new Date()).getFullYear();
    let month = (new Date()).getMonth() + 1;
    let date = (new Date()).getDate();
    console.log('testtest ===> ', year, month, date);

    if (month + recurringPlusMonth > 12) {
      year += 1;
      month = (month + recurringPlusMonth) % 12;
    } else {
      month += recurringPlusMonth;
    }
    ...
    <View>
      <View paddingLeft={15} paddingRight={15} marginBottom={20}>
        ...
        <View marginTop={10}>
          ...
          <ModalSelector
            data={deliveryRequestMessage.map((el, index) => ({
              key: index,
              label: el
            }))}
      ...
      {/* 쿠폰 적용 View {s} */}
      {putSampleItems.length !== 0 || paymentType ? null : (
      ...
      <View paddingLeft={15} paddingRight={15}>
        ...
        <View>
          ...
          {props.isLandCheckNewInput || props.isLandCheckNewInput ? (
            <ViewRow justifyContent={'space-between'} marginTop={10}>
              <Text ftGray fontSize={13}>
                도서/산간 배송비
              </Text>
