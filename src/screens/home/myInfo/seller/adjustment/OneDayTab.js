...
// Screen Component
import AdjustmentGraph from './AdjustmentGraph';
import {LocaleString} from '../../../../../utils/functions';
// NPM Module
import Moment from 'moment';

const OneDayTab = (props) => {
  // const [profitButton, setProfitButton] = useState(false);
  //
  // const onPressProfitButton = () => {
  //   if (profitButton) setProfitButton(false);
  //   else setProfitButton(true);
  // };

  return (
    <>
      {props.dayAdjustmentInfo ? (
        <>
          {props.dayAdjustmentInfo.adjustmentInfo.length !== 0 ? (
            <View>
              <View bgWhite paddingLeft={20} paddingRight={20}>
                <ViewRow
                  ...
                  <ViewBorderRadius>
                    <Text ftGray>
                      {Moment(
                        props.dayAdjustmentInfo.adjustmentInfo[0].assignedAt,
                  ...
                  <Text ftDarkNavy bold fontSize={20}>
                    {LocaleString(
                      props.dayAdjustmentInfo.adjustmentInfo[0].profit,
                    )}
                    원
                  </Text>
                </ViewRow>
                ...
                <ViewRow
                  ...
                  <ViewRow alignItems={'center'}>
                    <Text ftDarkNavy fontSize={15} bold>
                      {LocaleString(
                        props.dayAdjustmentInfo.adjustmentInfo[0].revenue,
                      )}
                      원
                    </Text>
                  </ViewRow>
