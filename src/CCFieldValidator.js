...
export default class CCFieldValidator {
  ...
  validateValues = (formValues) => {
    ...
    const birthValidation = valid.cvv(formValues.birth, 6);
    const passwordValidation = valid.cvv(formValues.password, 2);
    ...
    const validationStatuses = pick({
      ...
      birth: toStatus(birthValidation),
      password: toStatus(passwordValidation),
